...
- [ ] **사이트맵 제출 + 색인 생성 요청** — 속성 등록만으로는 색인되지 않습니다. Search Console에서 두 가지를 더 해야 합니다.
  ...
  빌드 쪽은 막는 게 없습니다 — `robots.txt`가 `Allow: /`, 사이트맵에 42개 URL(도구·변환 쌍·가이드 포함)이 전부 새 주소로 들어 있고, 홈·도구 페이지에 `noindex`가 없습니다(`npm test`가 확인). URL을 2026-08-13에 `/{언어}/{슬러그}/`로 바꿨으므로 지금 제출하면 처음부터 최종 주소로 들어갑니다.
