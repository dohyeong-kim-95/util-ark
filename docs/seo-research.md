...
Utilark는 `/ko/tools/merge-pdf/`로 3개였고 `tools/` 세그먼트가 하는 일이 없었습니다. **2026-08-13에 제거해 `/ko/merge-pdf/`가 됐습니다**. 순위 요인으로서 URL 깊이의 비중은 크지 않지만, **색인이 거의 쌓이지 않은 지금이 바꾸기 가장 싼 시점**입니다. 나중에 바꾸면 리다이렉트를 유지해야 합니다.
...
5. ~~**URL에서 `tools/` 제거 여부 결정**~~ — 2026-08-13 제거 완료. 옛 경로는 Worker가 301로 보냅니다.
