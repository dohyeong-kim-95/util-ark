...
네이버는 구글 색인을 쓰지 않고 자체 색인을 운영합니다. **Search Console 등록은 네이버 노출에 아무 영향을 주지 않습니다.** 조사한 키워드(`글자수 세기`·`PDF 합치기`·`사다리타기`)는 국내 검색 비중이 크므로 [서치어드바이저](https://searchadvisor.naver.com)를 따로 봐야 합니다.

다만 구글과 전제가 다릅니다. 네이버 도움말은 이렇게 말합니다.
...
즉 **등록이 노출의 전제 조건은 아닙니다.** 등록의 값은 두 가지입니다 — 로봇에게 사이트 존재를 알리는 가장 쉬운 방법이라는 것(구글의 Search Console 제출과 같은 역할), 그리고 수집·색인·노출 리포트를 볼 수 있다는 것. 리포트는 소유확인을 마쳐야 열립니다.

소유확인 제약은 구글에 없는 조건들이라 따로 챙겨야 합니다. 태그가 `<head>` 밖이나 frame 안에 있으면 검증에서 제외되고, 검증기가 자바스크립트를 실행하지 않으며, 브라우저단 리다이렉션(JS·`meta refresh`·쿠키)은 지원하지 않습니다. **서버측 HTTP 301·302는 지원합니다** — 이 사이트의 루트 302가 여기 해당합니다. `npm test`가 배치 규칙을 검사합니다.
